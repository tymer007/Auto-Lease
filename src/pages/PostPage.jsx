...
const PostPage = () => {
  ...
  const [profile, setProfile] = useState({
    name: "",
    description: "",
    coverImage: "/default-profile.png",
  });
    ...
    const fetchProfileData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `https://auto-lease-backend.onrender.com/api/v1/dealerships/${userIdFromToken}`,
          {
            headers: {
      ...
      <section className="container mx-auto mt-8 p-4">
        ...
        <div className="grid grid-cols-3 gap-8 p-6">
          {loading ? (
            ...
            <div className="min-h-72 flex text-center">
              <p className="min-h-72 text-center">No cars available.</p>
            </div>
          )}
        </div>
      </section>
...
const CarCard = ({ category, name, image, price, description }) => {
  return (
    <div className="border-2 border-gray-300 rounded-xl p-4 bg-cream">
      <div className="mb-4">
        <p className="text-primary-purpler border-2 border-primary-purpler bg-transparent w-20 h-10 rounded-lg text-lg font-medium text-center pt-1">
          {category}
        </p>
      ...
      <div className="text-center">
        <button className="bg-primary-purpler text-white rounded-full w-40 h-10 mb-2">
          Available Rent
        </button>
        <button className="bg-transparent text-primary-purpler border-2 border-primary-purpler rounded-full w-40 h-10">
          Unavailable
        </button>
      </div>
